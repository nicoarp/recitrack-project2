...
app.post('/api/qr/resolve', async (req, res) => {
  try {
    ...
    if (qrRecord) {
      // Elegir ruta de redirección según tipo de evento
      let redirectUrl = `/batch-validation?qrId=${qrRecord.qrId}`;
      if (qrRecord.eventType === 'Deposit') {
        redirectUrl = `/deposit-validation?qrId=${qrRecord.qrId}`;
      }
